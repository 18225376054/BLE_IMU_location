...
sl_status_t aoa_serialize_geograph(double latitude, double longitude, double altitude, char** str)
{
  cJSON *root = cJSON_CreateObject();
  cJSON_AddDoubleToObject(root, "latitude", latitude);
  cJSON_AddDoubleToObject(root, "longitude",longitude);
  cJSON_AddDoubleToObject(root, "altitude", altitude);
  *str = cJSON_Print(root);
  cJSON_Delete(root);
  return SL_STATUS_OK;
}
